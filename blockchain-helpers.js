...
function getContributions() {
  const contributionsFile = readFileSync("./contributions.json");
  const contributions = JSON.parse(contributionsFile);
  return contributions;
}

function writeContributions(contributions) {
  const contributionsString = JSON.stringify(contributions, null, 2);
  writeFileSync("./contributions.json", contributionsString);
}

const _getContributions = getContributions;
export { _getContributions as getContributions };
const _writeContributions = writeContributions;
export { _writeContributions as writeContributions };
...
function getAddressBalance(address) {
  ...
  for (let i = 1; i < blockchain.length; i++) {
    ...
    for (let j = 0; j < transactions.length; j++) {
      const { fromAddress, toAddress, amount = 0 } = transactions[j];
  ...
  for (let i = 0; i < transactions.length; i++) {
    const { fromAddress, toAddress, amount = 0 } = transactions[i];
    if (fromAddress === address) {
      balance -= amount;
    }
